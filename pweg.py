...
def convert_images(folder_path):
    ...
    for file_path in all_files:
        ...
        if var3.get():  # 如果包含子文件夹
            relative_path = os.path.relpath(src_folder, start=folder_path)
            dst_folder = os.path.join(target_folder, relative_path) if var1.get() else src_folder
        else:
            dst_folder = target_folder if var1.get() else src_folder
        
        os.makedirs(dst_folder, exist_ok=True)
        target_file = os.path.join(dst_folder, os.path.splitext(filename)[0] + f'.{target_ext}')
        ...
        try:
            ...
            if conversion_type == 'webp → png':
                ...
                convertPNGtoWebP(filename, src_folder, dst_folder, lossless_var.get())
            elif conversion_type == 'png → jpg':
                convertPNGtoJPEG(filename, src_folder, dst_folder, lossless_var.get())
            ...
            print(f"转换文件 {filename} 时发生错误: {str(e)}")
